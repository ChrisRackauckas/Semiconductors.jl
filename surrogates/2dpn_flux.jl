...
function diode_setup(d)
	...
	doping_initial = zero(d.grid.components[Coordinates][1,: ])
	for (i,x) in enumerate(d.grid.components[Coordinates][1,: ])
		if x<-1e-9
...
function main()
	...
	d_fine = diode2d(2.0,2.0,1.0,0.5,dx1=0.1,dx2=0.02,dx3=0.1,ny=9)
	d_coarse = diode2d(2.0,2.0,1.0,0.5,dx1=0.8,dx2=0.16,dx3=0.8)
	d_coarse_init = diode2d(2.0,2.0,1.0,0.5,dx1=0.8,dx2=0.16,dx3=0.8)
	...
	params_coarse.damp_initial = 0.1
	params_coarse.max_iters = 1000
	params_coarse.verbose = verbose_print_iters
	...
	theta_initial = zeros(1+length(d_coarse.grid[Coordinates][1,: ]))
	...
	d_fine = diode2d(2.0,2.0,1.0,0.5,dx1=0.1,dx2=0.05,dx3=0.1)
	d_coarse_init = diode2d(2.0,2.0,1.0,0.5,dx1=0.2,dx2=0.1,dx3=0.2)
